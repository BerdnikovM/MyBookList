...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $title = $_POST['title'];
    $genre = $_POST['genre'];
    $author = $_POST['author']; // Получаем автора книги
    $publication_date = $_POST['publication_date'];
    $annotation = $_POST['annotation'];
    $imagePath = $_POST['imagePath']; // Получаем путь к загруженному изображению

    // Проверяем, что все данные были получены
    if (empty($title) || empty($genre) || empty($author) || empty($publication_date) || empty($annotation) || empty($imagePath)) {
    ...
    $stmt = $conn->prepare("INSERT INTO books (title, genre, Author, publication_date, annotation, book_img_path) VALUES (?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
    ...
    $stmt->bind_param("ssssss", $title, $genre, $author, $publication_date, $annotation, $imagePath);
    $result = $stmt->execute();
    if (!$result) {
